...
/// service restarts. Scheduled incrementals are blocked until a FULL session is
/// completely extracted and all of its queued batches are acknowledged.
/// </summary>
public sealed class SyncSessionRepository
{
    ...
    private void EnsureSchema()
    {
        ...
        cmd.CommandText = """
            ...
            CREATE INDEX IF NOT EXISTS ix_prod_acks_sync
              ON production_session_acks(sync_id);
            """;
        cmd.ExecuteNonQuery();
    }
    ...
    /// <summary>
    /// Registers the sync ID already generated by SyncEngine. INSERT OR IGNORE
    /// makes restart/re-entry safe and allows upload acknowledgements to arrive
    /// before extraction finishes.
    /// </summary>
    public void Register(string syncId, string company, string syncType, string? startedUtc = null)
    {
        using var conn = _db.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            INSERT OR IGNORE INTO production_sync_sessions
              (sync_id,company,sync_type,status,started_utc,batches_acknowledged)
            VALUES (
              $id,$c,$t,$s,$now,
              (SELECT COUNT(*) FROM production_session_acks WHERE sync_id=$id)
            )
            """;
        ...
        cmd.Parameters.AddWithValue("$now", startedUtc ?? DateTime.UtcNow.ToString("O"));
        cmd.ExecuteNonQuery();
    }
    ...
    public SyncSession? GetLatest(string company)
    {
        using var conn = _db.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            SELECT * FROM production_sync_sessions
            WHERE company=$c ORDER BY started_utc DESC LIMIT 1
            """;
        cmd.Parameters.AddWithValue("$c", company);
        using var r = cmd.ExecuteReader();
        return r.Read() ? Map(r) : null;
    ...
    public void MarkExtractionCompleted(string syncId, long rows, int failedBatches, string? error)
    {
        using var conn = _db.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            UPDATE production_sync_sessions SET
              ...
              rows_extracted=$rows,
              batches_queued=(
                SELECT COUNT(*) FROM upload_batches WHERE sync_id=$id
              ) + (
                SELECT COUNT(*) FROM production_session_acks WHERE sync_id=$id
              ),
